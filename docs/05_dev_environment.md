...
.venv/bin/python manage.py set_email_logo ~/path/to/logo.png
.venv/bin/python manage.py set_email_logo --clear                         # back to the practice name
.venv/bin/python manage.py set_email_logo ~/path/to/mark.png --mark        # the square mark beside a personal sign-off (56 × 56 px)
```

The sign-off block uses each person's signature from **Settings → Sender** (name on the first line, then title, phone, email, website — one per line).

A preview shows the logo embedded; a real send carries it as an inline image part. If the file cannot be read at send time, the header shows the practice name and the email still goes.
...
It declares every schedule in one list (`apps/tenancy/management/commands/ensure_schedules.py`): referral-touch drafting daily at 06:00 tenant time, Outbox expiry and contact reindex hourly, note processing every minute, audio retention daily, and Module 3's one tick every minute (digest quiet windows, generation, expiry, sending, and the client-activity notice). Re-running never moves a schedule that is on cadence — but it **realigns one that has fallen more than one interval behind** (a cluster stopped for days), putting a daily job back on its proper local hour rather than firing it at whatever time the cluster came back.

**A stale schedule no longer needs a hand-run, though.** `catch_up` is **off** (`Q_CLUSTER`, `config/settings.py`): Django-Q advances a behind schedule past now and fires it **once**, so a laptop that slept overnight resumes cadence by itself. With `catch_up` on — the setting until 2026-09-16 — it advanced a stale next run by a single interval per scheduler pass instead, so a one-minute job left nine hours behind fired every ~30 seconds for hours and its next run stayed in the past the whole time. *(Phase 3: `work.tick` was stuck at 12 Sep; on 16 Sep every minute-and-hourly schedule was again hours behind after an overnight sleep, which is what took the setting off.)* Until Phase 2 nothing registered a schedule at all.
