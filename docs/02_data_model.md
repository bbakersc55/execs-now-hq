...
| `email_logo` / `email_logo_width` / `email_logo_height` | FK → `stored_file` (nullable, SET NULL) / smallint / smallint | The email header logo (PNG or JPEG, `purpose = email_logo`) and its **display** size, fitted into 260 × 84 px when set by `manage.py set_email_logo`. Null: the header shows `email_display_name` (2026-09-15) |
