...
21a. **Session prep** *(owner, 2026-09-21)*. On a Draft session, the fractional gives the company's website and whatever they already know — pasted emails, call notes — and **one Claude call with web search enabled** returns four things: what the company does and how it sells; the ordinary bottlenecks for a business of that shape; **a suggested rewording of each pre-call question** in this prospect's own vocabulary, shown beside today's wording; and **five extra questions to ask live**. One `ai_call` per run, with its tokens, its cost, and the **number of searches**, which is recorded separately because Anthropic bills a search on top of tokens and a cost that quietly omitted them would be wrong in the direction that flatters us.
21b. **Nothing it suggests is applied by the app.** A rewording is **copied into the template editor**, prefilled and unsaved, and the fractional saves it themselves; an extra question appears in the live view only once they **pin** it. The template and the session's snapshot are untouched by a prep run.
...
1b. **The folder's past is a choice, not a default.** *(Added 2026-09-22. Diagnosed on the owner's own folder: "Sync now" reported **0 waiting** on 167 readable notes.)* Drive's `changes.list` starts from a token meaning **"now"**, so a freshly watched folder is, to the poller, empty until the next meeting happens. Reading what is already there is a separate thing, because it costs one Claude call per note and fills the review queue with months of work that may be long settled. So on connecting, the app **states what the folder already holds** — the readable count and the date range — and offers three, with **the count and an estimated AI cost shown before confirming**:
   ...
   **The offer stays while unread history remains.** *(Added 2026-09-22 from real use: an import that read 8 of 167 notes left 159 unread, and the panel congratulated itself and disappeared.)* Every time the queue is opened it says how many readable notes remain unimported and their date range, and offers the same three choices, each priced against what is *left*. The watched-folder card says the same thing where somebody wondering why nothing arrives will read it: **"watching for new notes; N older notes not imported — import them here"**, because "Sync now" correctly does nothing for files older than the cursor and that is not obvious. It goes quiet only when nothing is left unread.

   The chosen import runs **paced and oldest first**: a few notes a minute, so the cluster keeps serving the app, the queue fills in the order the engagement happened, and **the spend is visible while it runs** rather than arriving afterwards as one number. Each file is **recorded and then parsed, in that order** — the poll's own two-step commit, not a second ingestion path — and every one lands in the same review queue under the same rules. Stopping is one click and **keeps everything already read**. A second run can only ever pick up what the first left out, on the same `(tenant, file, version)` rule the poll uses.
1c. **The watcher descends one level into subfolders.** A folder per client, or per month, is a normal way to keep notes, and a watcher reading only direct children finds nothing in those. One level and no more: two would be a crawl of somebody's whole Drive from a single pasted link. The connect screen **names the subfolders it found** and how many readable notes are in them.
...
8d. **Call notes are readable on the record, not merely present on it.** *(Added 2026-09-22 from real use: the Meeting existed on the timeline and there was nowhere to read what was said.)* A contact's page carries a **Call notes** section — every meeting they attended, newest first, with the date, **the accepted summary or nothing** (FR-5.8b: a discarded summary is absent, never replaced by an invented one), who else was there, which of the practice attended, and a link to the source Google Doc so the record is checkable. The company page carries the same section, aggregating its contacts' meetings — aggregated rather than stored, so a contact who moves takes their history. **No client-facing surface** (matrix 11.1), including for a meeting about their own company.
...
9e. **A participant who is on the practice's own staff is recognised, not asked about.** *(Added 2026-09-22, from real proposals: the FF came back as a participant in **every** meeting, asking whether he was new and "what they are to us" — with only the five contact types offered, none of which is true. The practice is not a prospect, a client, a referral partner, a vendor or a coworker of itself. The question was wrong, not the answer.)*
   ...
   - A recognised participant is **shown** on the proposal (who was in the room is the point of the record), carries **no contact type**, offers **no candidates**, and **needs no approval**. It creates and changes nothing: no contact, no type, no pipeline stage, no send.
