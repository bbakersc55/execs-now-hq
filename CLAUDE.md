...
- **Migrations: SQL first, always; applying is conditional.** Show every migration's SQL (`manage.py sqlmigrate`) before generating it. You may then **apply it yourself, immediately**, only when all three hold:
  ...
  3. **A backup ran today** (`scripts/backup_db.sh`). The owner's session-start routine runs it in the terminal before anything else, every day, so this is normally already true before you start. **The owner's word in the conversation settles it** — do not re-run it to check, and do not ask twice. Run one yourself only when the owner has not confirmed today's, or when significant data has changed since it (a CSV import, a bulk edit, a restore). Never assume it: unconfirmed and unrun means the migration waits.
